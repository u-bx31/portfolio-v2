...
export const Navbar = ({ children, className }: NavbarProps) => {
	...
	return (
		<motion.div
			...
			className={cn(
				"sticky inset-x-0 top-10 z-40 w-full",
				className
			)}>
		...
		<motion.div
			...
			{items.map((item, idx) => (
				<a
					...
					onClick={onItemClick}
					className="relative px-4 py-2 text-text dark:text-neutral-300"
					key={`link-${idx}`}
					href={item.link}>
					{hovered === idx && (
						<motion.div
							layoutId="hovered"
							className="absolute inset-0 h-full w-full rounded-full bg-primary/40 dark:bg-neutral-800"
						/>
					)}
...
export const MobileNav = ({
	...
	return (
		<motion.div
			...
			className={cn(
				"relative z-50 mx-auto flex !rounded-lg w-full max-w-[calc(100vw-2rem)] flex-col items-center justify-between bg-transparent px-0 py-2 lg:hidden",
				visible && "bg-white/80 dark:bg-neutral-950/80",
				className
			)}>
			{children}
...
export const NavbarButton = ({
	...
	return (
		<Tag
			href={href || undefined}
			className={
				"bg-accent px-5 py-2 rounded-full text-lg font-normal relative cursor-pointer hover:-translate-y-0.5 transition duration-200 inline-block text-center shadow-sm " +
				className
			}
			{...props}>
			{children}
